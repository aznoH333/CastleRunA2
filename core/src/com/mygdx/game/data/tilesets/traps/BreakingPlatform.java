    ...
    public void draw(float x, float y) {
        if(isOn && flickerTimer % 4 < 2){
            if (!isInAStateOfCollapse)
                ...
                if (timer < 10)     DrawingManager.getINSTANCE().draw("platform5",x,y,1);
                else if (timer < 20) DrawingManager.getINSTANCE().draw("platform4",x,y,1);
                else if (timer < 30) DrawingManager.getINSTANCE().draw("platform3",x,y,1);
                else if (timer < 40) DrawingManager.getINSTANCE().draw("platform2",x,y,1);
                else if (timer < 50) DrawingManager.getINSTANCE().draw("platform1",x,y,1);
            }
        }
    }
